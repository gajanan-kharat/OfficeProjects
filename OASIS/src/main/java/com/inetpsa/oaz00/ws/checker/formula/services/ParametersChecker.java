...
public class ParametersChecker {
     ...
     * 
     * @param motherRequirement MotherRequirement from the RequirementList.
     * @param calculation Calcution object
    ...
    /**
     * To check unreferenced contributor present in the report formula.
     * 
     ...
     */
    public static boolean checkUnReferencedContributors(SciMotherRequirementType motherRequirement, Calculation calculation, boolean errorDetected) {
        // Hash set to collect all of the valid contributor-names.
        Set<String> contributorNames = new HashSet<String>();
        // Get the contributors name from motherRequirement.
        for (SciRequirementType sciContributor : motherRequirement.getContributorList()) {
        ...
        while (matcher.find()) {
            ...
            if (!ecFunctions.isFunction(var) && !ecConstants.isConstant(var)) {
                // ...and if not, search for it in the hash set to figure out if it is a valid contributor.
                if (!contributorNames.contains(var)) {
                    int unrefContributorPosition = getUnrefContributorPosition(var, calculationFormula, reportFormula);
                    // If the name is not present, it's an error - bad contributor name in the calculation formula!
                    if (!errorDetected) {
                        calculation.getResult().setStatus(ResultCodeType.INVALID_CONTRIBUTOR_REFERENCE.value());
                        calculation.getResult().setDescription(HALF_CONNECTOR_PROPERTIES.getProperty("INVALID_CONTRIBUTOR_REFERENCE"));
                        ...
                        // Now we need to figure out the position of the invalid contributor in the report formula and set the position in the result
                        // object.
                        calculation.getResult().setPosition(BigInteger.valueOf(unrefContributorPosition + 1));
                        calculation.getResult().setLevel(CriticalityLevels.ERROR.criticalityValue());
                        return true;
                    }
                }
    ...
    /**
     * To get the unreferenced contributor position from the report formula.
     * 
     ...
     * @author E495366:Gajanan Kharat
     */
    private static int getUnrefContributorPosition(String contributor, String calculationFormula, String reportFormula) {
        int indexOf = calculationFormula.indexOf(contributor);
        String formulaEncoded = calculationFormula.replaceAll("[A-Z]{2}-", "@@@");
        char[] charArrayFormula = formulaEncoded.toCharArray();
        List<Character> formulaOprSequences = new ArrayList<Character>();

        List<Character> listOfMathOprsInChars = new ArrayList<Character>();
        // Getting list of String mathematical operators and putting into the list Character.
        for (String operator : Constants.listOfMathOprs) {
            listOfMathOprsInChars.add(operator.charAt(0));
        }
        // To get the operator sequence from the report formula.
        for (int i = 0; i < indexOf; i++) {
            if (listOfMathOprsInChars.contains(charArrayFormula[i])) {
                formulaOprSequences.add(charArrayFormula[i]);
            }
        }
        int formulaOprSeqSize = formulaOprSequences.size();
        int oprSeq = 0;
        int invalidCharPosition = 0;
        char[] charArrayReport = reportFormula.toCharArray();
        // Operator sequence one to one mapping between formula and report formula for finding invalid unreferenced contributor from report formula.
        for (int i = 0; i < charArrayReport.length; i++) {
            if (listOfMathOprsInChars.contains(charArrayReport[i])) {
                oprSeq++;
                if (formulaOprSeqSize == oprSeq) {
                    invalidCharPosition = i + 1;
                    break;
                }
            }
        }
        System.out.println("Position:" + invalidCharPosition);
