...
import com.inetpsa.oaz00.ws.report.services.impl.ArithStatReportServiceImpl;
...
import com.inetpsa.oaz00.ws.scilab.services.impl.ArithStatSciCalculationServiceImpl;
import com.inetpsa.oaz00.ws.scilab.utils.ArithStatOutputAdapter;
...
/**
 * The Class ArithStatStrategyServiceImpl.
 * 
 * @author U224106
 */
public class ArithStatStrategyServiceImpl implements StrategyService {
    ...
    /** The logger. */
    private static Logger logger = Logger.getLogger(ArithStatStrategyServiceImpl.class);
    ...
    /**
     * Instantiates a new Arithmetic Statistical strategy service implementation.
     * 
     * @param calculation the calculation object
     */
    public ArithStatStrategyServiceImpl(Calculation calculation, Date execDate) {
    ...
    public void organizeInput() {
        ...
        if (pMotherRequirementsList == null) {
            this.calculation.getResult().setStatus(ResultCodeType.ERRONEOUS_ARITH_STAT_OUTPUT.value());
            this.calculation.getResult().setDescription(HALF_CONNECTOR_PROPERTIES.getProperty("ERRONEOUS_ARITH_STAT_OUTPUT"));
    ...
    public void checkInput() {
        ...
        for (TransferModelType transferModel : transferModels) {
            ...
            try {
                ...
                logger.warn("Arith Stat - Incorrect formula - Formula = " + formula + " - ProjectName = " + calculation.getProjectName() + " - UserId = " + calculation.getUserId(), ee.getCause());
            ...
            errorDetected = ParametersChecker.checkUnReferencedContributors(this.motherRequirement, this.calculation, errorDetected);
            /** VCOPLMPSA-37828: Code modification END. Date: 08-November-2016 */
        }
        // Getting TNC from the motherRequirement.
        Double tnc = this.motherRequirement.getTnc();
        // Check the TNC and report error, if any.
        if (tnc == null || tnc.equals(Constants.EMPTY_STRING) || tnc < 0) {
            // Check whether error was detected or not.
            if (!errorDetected) {
                // Setting the required field in ResultType
                this.calculation.getResult().setStatus(ResultCodeType.INVALID_ARITH_STAT_INPUT.value());
                this.calculation.getResult().setDescription(HALF_CONNECTOR_PROPERTIES.getProperty("INVALID_ARITH_STAT_INPUT"));
        ...
        // Getting the contributorsList from motherReqiurement and iterating over each contributor.
        for (SciRequirementType sciContributor : this.motherRequirement.getContributorList()) {
            ...
            if (nominalValue == null || nominalValue.equals(Constants.EMPTY_STRING) || lowerValue == null || lowerValue.equals(Constants.EMPTY_STRING) || upperValue == null
                    || upperValue.equals(Constants.EMPTY_STRING)) {
                // Check whether error was detected or not.
                if (!errorDetected) {
                    // Setting the required field in ResultType
                    this.calculation.getResult().setStatus(ResultCodeType.INVALID_ARITH_STAT_INPUT.value());
    ...
    public void runCalculation() {
        ...
        SciCalculationService calculationService = new ArithStatSciCalculationServiceImpl(motherRequirement);
        ...
        if (d.length == 0) {
            ...
            ArithStatOutputAdapter.createArithStatScilabResult(motherRequirement, d);
        }
    ...
    public void generateReport() throws IOException {
        ReportService reportService = new ArithStatReportServiceImpl(motherRequirement, calculation.getProjectName(), executionDate, calculation.getUserId());
    ...
    /**
     * Checks the output coming from Scilab.
     * 
    ...
    public void checkOutput() {
            ...
            for (RequirementType requirement : this.calculation.getRequirements().getRequirement()) {
                if (requirement.getId().equalsIgnoreCase(this.motherRequirement.getId())) {
                    ...
                    if (requirement.getTnc() < 100) {
                        ...
                        this.calculation.getResult().setStatus(ResultCodeType.ARITH_STAT_TNC_INPUT.value());
                        this.calculation.getResult().setDescription(HALF_CONNECTOR_PROPERTIES.getProperty("ARITH_STAT_TNC_INPUT"));
                        this.calculation.getResult().setImpactedObject(this.motherRequirement.getName());
        ...
        if (this.calculation.getResult().getStatus().longValue() == ResultCodeType.FORMULA_OK.value().longValue()) {
            // Setting the description for ArithStat.
            this.calculation.getResult().setDescription(HALF_CONNECTOR_PROPERTIES.getProperty("ARITH_STAT_COMPLETE"));
    ...
    private boolean setOutputErrorAttributes(boolean errorDetected) {
        ...
        if (!errorDetected) {
            // Setting the status, description, impactedObject and level.
            this.calculation.getResult().setStatus(ResultCodeType.INFINITE_ARITH_STAT_OUTPUT.value());
            this.calculation.getResult().setDescription(HALF_CONNECTOR_PROPERTIES.getProperty("INFINITE_ARITH_STAT_OUTPUT"));
            this.calculation.getResult().setImpactedObject(this.motherRequirement.getName());
    ...
    public Calculation prepareOutput() {
        ...
        for (RequirementType aRequirement : requirementsFromCalculation) {
            ...
            if (aRequirement.getParentId().equalsIgnoreCase(aRequirement.getId()) && aRequirement.getId().equalsIgnoreCase(this.motherRequirement.getId())) {
                ...
                // Setting the ArithmeticalIT, ArithmeticalValueInf, ArithmeticalValueSup, ArithmeticalITInf and ArithmeticalITSup values of the
                ...
                // Setting the StatisticalIT, StatisticalValueInf, StatisticalValueSup, StatisticalITInf and StatisticalITSup values of the
                // OriginalMotherRequirement.
    ...
    public static void main(String[] args) throws IOException, JAXBException {
        ...
        // Arithmetic Statistical calculation (ex-COTE)
        System.out.println("Start Arithmetic Statistical ...");

        JAXBContext jc = JAXBContext.newInstance(Calculation.class);

        Unmarshaller unmarshaller = jc.createUnmarshaller();
        File scenarioFile = new File("C:\\User\\SIPP\\PLM\\GP5\\Interface OASIS\\GL\\Test data\\Base\\Scenario 6\\Flow1_Scenario6B.xml");
        ...
        ArithStatStrategyServiceImpl arithStatStrategyServiceImpl = new ArithStatStrategyServiceImpl(calculation, Calendar.getInstance().getTime());

        arithStatStrategyServiceImpl.organizeInput();
        arithStatStrategyServiceImpl.runCalculation();
        arithStatStrategyServiceImpl.generateReport();

        System.out.println(arithStatStrategyServiceImpl.motherRequirement.toString());

        System.out.println("End Arithmetic Statistical ...");
    }
}
