...
public class SemiQuadStrategyServiceImpl implements StrategyService {
    ...
    public void checkInput() {
        ...
        for (TransferModelType transferModel : transferModels) {
            ...
            try {
                ...
                logger.warn("Semi Quad - Incorrect formula - Formula = " + formula + " - ProjectName = " + calculation.getProjectName() + " - UserId = " + calculation.getUserId(), ee.getCause());
        ...
        for (SciRequirementType sciContributor : this.motherRequirement.getContributorList()) {
            ...
            if (nominalValue == null || nominalValue.equals(Constants.EMPTY_STRING) || lowerValue == null || lowerValue.equals(Constants.EMPTY_STRING) || upperValue == null
                    || upperValue.equals(Constants.EMPTY_STRING) || mean == null || mean.equals(Constants.EMPTY_STRING) || standardDeviation == null
                    || standardDeviation.equals(Constants.EMPTY_STRING)) {
                // Check whether error was detected or not.
                if (!errorDetected) {
                    // Setting the required field in ResultType
                    this.calculation.getResult().setStatus(ResultCodeType.INVALID_SEMI_QUAD_INPUT.value());
    ...
    public void generateReport() throws IOException {
        ReportService reportService = new SemiQuadReportServiceImpl(motherRequirement, calculation.getProjectName(), executionDate, calculation.getUserId());
