 ...
 */
public class MVEGRule2Calculator implements DecisionCalculator {
	 ...
	 */
	public MVEGRule2Calculator(PollutantGasResultSetRepository pgResultSetRepository) {
	 ...
	 */
	private String getDecisionForResult(double statisticalResult, Integer integer) {
		...
		for (StatisticalRuleDataDto rule : rules) {
			if (ConstantsApp.MVEG_2.equalsIgnoreCase(rule.getRuleName())) {
				ValueDto valuedto = rule.getValuesDtoMap().get(integer);

				if (statisticalResult < valuedto.getValueAn())
					decision = ConstantsApp.DECISION_A + integer;
				else if (statisticalResult > valuedto.getValueBn())
					decision = ConstantsApp.DECISION_R + integer;
				else if (valuedto.getValueAn() < statisticalResult && statisticalResult < valuedto.getValueIn())
					decision = ConstantsApp.DECISION_I1 + "-" + integer;
				else
					decision = ConstantsApp.DECISION_I2 + "-" + integer;
			}
		}
		return decision;
	 ...
	 */
	private Map<String, Double> getAllVnValues(Map<String, ArrayList<Double>> resultValue, Map<String, Double> allDnValues, int i) {
		Map<String, Double> vnValues = new HashMap<>();
		for (Entry<String, ArrayList<Double>> resultEntry : resultValue.entrySet()) {
			Double vnValue = CalculationUtil.calculateStandardDeviation(resultEntry.getValue(), allDnValues.get(resultEntry.getKey()), i);
	...
	private Map<String, Double> getAllDnValues(Map<String, ArrayList<Double>> resultValue) {
		...
		for (Entry<String, ArrayList<Double>> resultEntry : resultValue.entrySet()) {
			double averageValue = CalculationUtil.calculateAverage(resultEntry.getValue());
			dnValues.put(resultEntry.getKey(), averageValue);
		}
		return dnValues;
	}
	...
	private String getGlobalDecision(List<String> resultValue) {
		...
		String decision = null;
		boolean isResultI2 = false;
		for (String result : resultValue) {
			if (result.indexOf(ConstantsApp.DECISION_R) != -1)
				return ConstantsApp.DECISION_R;

			else if (result.indexOf(ConstantsApp.DECISION_I2) != -1)
				isResultI2 = true;
			else if (result.indexOf(ConstantsApp.DECISION_I1) != -1)
				decision = ConstantsApp.DECISION_I1;
		}
		if (isResultI2)
			decision = ConstantsApp.DECISION_I2;
		else if (ConstantsApp.DECISION_I1.equalsIgnoreCase(decision))
			decision = ConstantsApp.DECISION_I1;
		else
