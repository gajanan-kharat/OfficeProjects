...
/**
 * This class provide application rights management form.
 * 
 * @author Geometric Ltd.
 */
public class EdsFormRightManagementForm extends A_EdsEditForm {
    // PUBLIC
    /**
     ...
     */
    public EdsFormRightManagementForm(EdsApplicationController controller) {
    ...
    public boolean commitChanges() {
        for (EdsFormRightAffectationEditForm form : editFormsMap.values()) {
            form.commitChanges();
        }
        return true;
    }
    ...
    /**
     * Variable to hold map of EdsRole and EdsFormRightAffectationEditForm
     */
    private HashMap<EdsRole, EdsFormRightAffectationEditForm> editFormsMap;
    /**
    ...
    /**
     * Initialize form right management form
     */
    private void init() {
        this.editFormsMap = new HashMap<EdsRole, EdsFormRightAffectationEditForm>();
        TabSheet vTSmain = new TabSheet();
        vTSmain.setSizeFull();

        vTSmain.addTab(getContentLayout(), controller.getBundle().getString("Admin-droit-form-title"));

        addComponent(vTSmain);
    }

    /**
     * This method provide layout for form right management form
     * 
     * @return Layout for form right management form
     */
    ...
    private Layout getRoleForm(EdsRole role) {
        ...
        if (form == null) {
            form = new EdsFormRightAffectationEditForm(role, controller);
            editFormsMap.put(role, (EdsFormRightAffectationEditForm) form);
            ((EdsFormRightAffectationEditForm) form).discardChanges();
        }
        return form;
    }
}
