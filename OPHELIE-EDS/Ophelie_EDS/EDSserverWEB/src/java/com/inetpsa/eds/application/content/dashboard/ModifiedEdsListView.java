...
import com.inetpsa.eds.tools.filter.fixedmodifdate.EdsFixedModifDateFilter;
...
/**
 * This view shows a table lists EDS sheets and a date picker to filter the list of records displayed as a function of their modification date outside
 * a time interval.
 * 
 * @author Geometric Ltd.
 * @see WeekTimePicker, UnmodifiedEdsList
 */
public class ModifiedEdsListView extends VerticalLayout {
    // PUBLIC
    /**
     ...
     */
    public ModifiedEdsListView(EdsApplicationController controller) {
    ...
    /**
     * Variable to hold value of EdsModifDateFilter
     */
    private EdsModifDateFilter modifFilter;

    /**
     * Initialize Modified Eds list view
     */
    private void init() {
        ...
        Button vBTrefresh = new Button(null, new Button.ClickListener() {
            public void buttonClick(ClickEvent event) {
                modifFilter.setInterval(timePicker.getSelectedStartDate(), timePicker.getSelectedEndDate());
        ...
        modifFilter = new EdsFixedModifDateFilter(timePicker.getSelectedStartDate(), timePicker.getSelectedEndDate(), controller);

        this.filterPanel.getFilterManagerView().addFilter(modifFilter);
    }
}
