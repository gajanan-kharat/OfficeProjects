...
/**
 * This class provide Activation nominal current theoretical panel
 * 
 * @author Geometric Ltd.
 */
public class CourantNominalActivationTheorique extends Panel {
    // PUBLIC
    /**
     ...
     */
    public CourantNominalActivationTheorique(EdsConsolidateSupplyTheoritic ecst, EdsQcf qcfConsolide, EdsApplicationController controller) {
    ...
    /**
     * Initialize Activation nominal current theoretical panel
     */
    private void init() {
        ...
        vBAdd.addListener(new Button.ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                ...
                CourantNominaleActivationFormEditView mmfev = new CourantNominaleActivationFormEditView(qcfConsolide, ecaa, ecaa.isCnaedsRemove(),
                        ecaa.isCnaedsIEfficace(), controller, ecst.getCsmedsConsolidateSupply().getCsedsTbtBt().equals("BT")) {
                    /*
                     * (non-Javadoc)
                     * 
                     * @see com.vaadin.ui.Button.ClickListener#buttonClick(com.vaadin.ui.Button.ClickEvent)
                    ...
                    public void buttonClick(ClickEvent event) {
                        ...
                        cnafevs.remove(this);
                        ecst.getEdsCourantNominaleActivations().remove(ecaa);
                    }
                };

                addCourantNominalActivation(mmfev, ecaa.isCnaedsRemove());

                ecst.getEdsCourantNominaleActivations().add(ecaa);
            }
        });
        addComponent(vBAdd);
    }
    ...
    /**
     * This method removes Activation nominal current theoretical
     * 
    ...
    /**
     * This method checks if Activation nominal current theoretical is valid
     * 
     * @return Check if Activation nominal current theoretical is valid
     */
    boolean isValid() {
        for (CourantNominaleActivationFormEditView cnafev : cnafevs) {
            if (!cnafev.isValid()) {
