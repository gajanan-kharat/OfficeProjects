/**
 * Famous last words: based on Java Observer
 * Creation : 19/08/2015
...
public class Observable<E> {
    ...
    private Vector<Observer<E>> obs;

    /** Construct an Observable with zero Observers. */

    public Observable() {
        obs = new Vector<Observer<E>>();
    }
    ...
    public void notifyObservers(E arg) {
        ...
        synchronized (this) {
            /*
             * We don't want the Observer doing callbacks into arbitrary code while holding its own Monitor. The code where we extract each Observable
             * from the Vector and store the state of the Observer needs synchronization, but notifying observers does not (should not). The worst
             * result of any potential race-condition here is that: 1) a newly-added Observer will miss a notification in progress 2) a recently
     ...
     * <tt>hasChanged</tt> method will now return <tt>false</tt>. This method is called automatically by the <code>notifyObservers</code> methods.
     * 
     ...
     * 
     * @return <code>true</code> if and only if the <code>setChanged</code> method has been called more recently than the <code>clearChanged</code>
