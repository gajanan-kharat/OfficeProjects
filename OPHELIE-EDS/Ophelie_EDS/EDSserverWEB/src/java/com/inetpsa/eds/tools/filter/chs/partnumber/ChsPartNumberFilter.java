...
/**
 * Filter plug according to a Part Number.
 * 
 * @author Joao Costa @ Alter Frame
 */
public class ChsPartNumberFilter extends A_EdsFilter {
    // PUBLIC
    /**
     ...
     */
    public ChsPartNumberFilter(String edsName, int filterType, EdsApplicationController controller) {
        super(controller);
    ...
    public String describe() {
        switch (filterType) {
        case EdsFilterManager.FILTER_EQUALS:
            return MessageFormat.format(controller.getBundle().getString("chs-filter-partnum-equal"), edsName);
        case EdsFilterManager.FILTER_ISLIKE:
            return MessageFormat.format(controller.getBundle().getString("chs-filter-partnum-in"), edsName);
        default:
            return null;
        }
    }
     ...
     */
    private static final Property name = Property.forName("partNumber");
    /**
